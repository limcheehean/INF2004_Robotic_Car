...
// Function to reverse the binary representation of a value
...
// Function to analyze and study interrupt values
    ...
    bool *refresh_buffers)
{
    ...
    static float old_length = 0;

    float new_length = new_isr_data->time_passed;
    *(new_isr_data->wheel_encoder_speed); //* new_isr_data ->  wheel_encoder_speed;
    if (old_length == 0)
    {
...
// Function to interpret barcode from the buffer
...
// Initialization function for the barcode interpretation task
void init_barcode_interpret_task()
{
    ...
    xTaskCreate(barcode_interpret_task,
                ...
                (void *)&g_barcode_interpret_queue,
                tskIDLE_PRIORITY,
